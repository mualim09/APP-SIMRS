...
{    
  ...
  if($num == 1){
    ...
    if($query){
    // var_dump($query);die();
      header("Location: ./mdr_working_paper_nak.php");                                                  
    } else {
...
<div class="modal fade" id="DASHBOARD-NAK<?php echo $row['id_logbook'];?>" role="dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="">
          ...
          <div class="col-lg-12">
            <div align="center">
              <hr><h4><b>Dashboard Kualitas NAK</b></h4><hr>
            </div>
          </div>
          ...
          <!-- /.form group -->
          <!-- <button type="submit" name="create_dh" value="submit" class="btn btn-success btn-block btn-flat">Submit</button> -->
...
<?php
$dashboard = mysql_query("SELECT COUNT(dashboard) AS jumlah_dashboard FROM tb_parameter WHERE id_logbook ='".$row['id_logbook']."' AND status_ccr='".$tipe_ccr."' AND dashboard!='blank' GROUP BY dashboard");
$row_dashboard = mysql_num_rows($dashboard);
// var_dump($row_dashboard);
$count = $row_dashboard*20;
?>
